...
//DAO: Date Access(접근-읽기와 쓰기) Object
//		ㄴ sql실행 메소드를 모아 놓은 클래스.

/*StudentDao 의 내용을 요약.
 * insert,update 는 sql 파라미터에 전달한 데이터의 타입을 dto
 * delete는										원시형 또는 String
 * delete sql 의 조건절 컬럼이 여러개 일때는 dto가 될 수 있습니다. map 도 종종 사용합니다.
 * 
...
public class StudentDao {
	...
	public List<StudentDto> selectAll() throws SQLException{
		...
		ResultSet rs = ps.executeQuery();
		while(rs.next()) {
				StudentDto dto = new StudentDto(rs.getString(1),rs.getNString(2),rs.getInt(3),rs.getString(4));
				results.add(dto);
			}
