	...
	public static void main(String[] args) {
		...
		while(!isLogin) {
			...
			try {
				...
				isLogin=true;			//반복종료
				}else System.out.println("입력하신 고객ID가 존재하지 않습니다.");
			} catch (SQLException e) {
		...
		if(isLogin) {
			while(true) {
				...
				System.out.print("구매할 상품 코드를 입력하세요. >>> ");
			...
			//6. 마이페이지 - 구매내역 보기. 총 구매 금액을 출력해 주기 -> sql 테스트 해보고 메소드 작성 시도하기
			System.out.println("\n" +"결제를 완료 했습니다. 현재까지 " +customer.getName() +" 회원님의 구매내역입니다.");
			JMypageDao mypage = new JMypageDao();
			try {
				//DecimalFormat df = new DecimalFormat("###,###,###");
			
				System.out.println("구매내역은 : " + mypage.Mypage(customid) + " 입니다.");
				System.out.println("총 구매 금액은 : " + mypage.myMoney(customid) + " 입니다.");
			} catch (SQLException e) {
